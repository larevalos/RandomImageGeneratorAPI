...
namespace EvaluationAPI.Filters
{
    public class LinkRewritingFilter : IAsyncResultFilter
    {
        ...
        //IUrlHelperFactory let us create IUrlHelpers on the fly
            ...
            ResultExecutingContext context, ResultExecutionDelegate next)
        {
            ...
            if (shouldSkip)
            {
                return next(); //allow the processing of the response to continue
            }

            //If not shuoldSkip  means that it's just a normal response. So we wanna look for any Link objects on the response.
        ...
        private static void RewriteAllLinks(object model, LinkRewriter rewriter)
        {
            ...
            //if not null, we will grab all the properties out of the model using reflection.
            ...
            foreach (var linkProperty in linkProperties)
            {
                ...
                //when it comes back as a new link
                linkProperty.SetValue(model, rewritten); //replace original link with the new link

                //handeling of the hidden Self property :
